...
//  bcm2835_gpio_write(RSTPIN, LOW);
    // init des gpio et spi en remplacement du code ci-dessus
    _gpio25 = new CGpio(nullptr, 25, OUT);
    _gpio25->ecrire(LOW);
    _spi = new CSpi(nullptr, '0',4000000, false, SPI_MODE_0);
...
//  bcm2835_spi_setChipSelectPolarity(BCM2835_SPI_CS0, LOW);      // the default
    // Code remplacé par CSpi et CGio
 ...
 */
void CMFRC522::PCD_WriteRegister(	byte reg,		///< The register to write to. One of the PCD_Register enums.
					byte value		///< The value to write.
					) {

  // bcm2835_spi_transfern(data, 2);
  unsigned char data[2]; // PhA ajout unsigned
  data[0] = reg & 0x7E;
  data[1] = value;
  _spi->ecrireNOctets(data, 2);
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
byte CMFRC522::PCD_ReadRegister(	byte reg	///< The register to read from. One of the PCD_Register enums.
				) {
  
  unsigned char data[2]; // PhA ajout unsigned
  data[0] = 0x80 | ((reg) & 0x7E);
  data[1] = 0;
//  printf("PCD_ReadRegister_01  %02X   %02X\n", data[0], data[1]);
  _spi->lireEcrire(data, 2);// bcm2835_spi_transfern(data,2);
//  printf("PCD_ReadRegister_02  %02X   %02X\n", data[0], data[1]);
  usleep(10000);
  return static_cast<byte>(data[1]);
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void CMFRC522::PCD_ReadRegister(	byte reg,		///< The register to read from. One of the PCD_Register enums.
				byte count,		///< The number of bytes to read
				byte *values,	///< Byte array to store the values in.
				byte rxAlign	///< Only bit positions rxAlign..7 in values[0] are updated.
				) {
  quint8 data[3]={0};
//  printf("PCD_ReadRegister___________NB  %02X\n", count);
  ...
  _spi->ecrireNOctets(&address,1); // bcm2835_spi_transfer(address);
  usleep(10000);
  while (index < count) {
    if (index == 0 && rxAlign) {		// Only update bit positions rxAlign..7 in values[0]
      ...
      data[0] = address;
      data[1] = 0;
//      printf("PCD_ReadRegister____11  %02X   %02X\n", data[0], data[1]);
      _spi->lireEcrire(data, 1);
//      printf("PCD_ReadRegister____12  %02X   %02X\n", data[0], data[1]);
      byte value = data[0];
      ...
      data[0] = address;
      data[1] = 0;
//      printf("PCD_ReadRegister____21  %02X   %02X\n", data[0], data[1]);
      _spi->lireEcrire(data,1);//
//      printf("PCD_ReadRegister____22  %02X   %02X\n", data[0], data[1]);
      values[index] = data[0];
    }
    index++;
    usleep(10000);
  ...
  data[0] = 0;
  data[1] = 0;
//  printf("PCD_ReadRegister____31 data1 %02X  data2 %02X\n", data[0], data[1]);
  _spi->lireEcrire(data,1);//
//  printf("PCD_ReadRegister____32 data1 %02X  data2 %02X\n", data[0], data[1]);
  values[index] = data[0];
  usleep(10000);
} // End PCD_ReadRegister()
 ...
 */
void CMFRC522::PCD_SetRegisterBitMask(	byte reg,	///< The register to update. One of the PCD_Register enums.
 ...
 */
void CMFRC522::PCD_ClearRegisterBitMask(	byte reg,	///< The register to update. One of the PCD_Register enums.
 ...
 */
byte CMFRC522::PCD_CalculateCRC(	byte *data,		///< In: Pointer to the data to transfer to the FIFO for CRC calculation.
 ...
 */
byte CMFRC522::PCD_GetAntennaGain() {
 ...
 */
void CMFRC522::PCD_SetAntennaGain(byte mask) {
...
bool CMFRC522::PCD_PerformSelfTest() {
  ...
  const byte *laReference=nullptr;
  PCD_Reset();
  ...
  case 0x91:	// Version 1.0
    laReference = MFRC522_firmware_referenceV1_0;
    break;
  case 0x92:	// Version 2.0
    laReference = MFRC522_firmware_referenceV2_0;
    break;
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte CMFRC522::PCD_TransceiveData(	byte *sendData,		///< Pointer to the data to transfer to the FIFO.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte CMFRC522::PCD_CommunicateWithPICC(	byte command,		///< The command to execute. One of the PCD_Command enums.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte CMFRC522::PICC_RequestA(byte *bufferATQA,	///< The buffer to store the ATQA (Answer to request) in
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte CMFRC522::PICC_WakeupA(	byte *bufferATQA,	///< The buffer to store the ATQA (Answer to request) in
				byte *bufferSize	///< Buffer size, at least two bytes. Also number of bytes returned if STATUS_OK.
				) {
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */ 
byte CMFRC522::PICC_REQA_or_WUPA(	byte command, 		///< The command to send - PICC_CMD_REQA or PICC_CMD_WUPA
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte CMFRC522::PICC_Select(	Uid *uid,			///< Pointer to Uid struct. Normally output, but can also be used to supply a known UID.
 ...
 */
byte CMFRC522::PCD_Authenticate(byte command,		///< PICC_CMD_MF_AUTH_KEY_A or PICC_CMD_MF_AUTH_KEY_B
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte CMFRC522::MIFARE_Read(	byte blockAddr, 	///< MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The first page to return data from.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte CMFRC522::MIFARE_Write(	byte blockAddr, ///< MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The page (2-15) to write to.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte CMFRC522::MIFARE_Ultralight_Write(	byte page, 		///< The page (2-15) to write to.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte CMFRC522::MIFARE_Decrement(	byte blockAddr, ///< The block (0-0xff) number.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte CMFRC522::MIFARE_Increment(	byte blockAddr, ///< The block (0-0xff) number.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte CMFRC522::MIFARE_Restore(	byte blockAddr ///< The block (0-0xff) number.
				) {
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte CMFRC522::MIFARE_TwoStepHelper(	byte command,	///< The command to use
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte CMFRC522::MIFARE_Transfer(	byte blockAddr ///< The block (0-0xff) number.
				) {
  byte result;
  byte cmdBuffer[2]; // We only need room for 2 bytes.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte CMFRC522::MIFARE_GetValue(byte blockAddr, long *value) {
  byte status;
  byte buffer[18];
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte CMFRC522::MIFARE_SetValue(byte blockAddr, long value) {
  byte buffer[18];
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
byte CMFRC522::PCD_MIFARE_Transceive(	byte *sendData,		///< Pointer to the data to transfer to the FIFO. Do NOT include the CRC_A.
 ...
 * 
 */
const string CMFRC522::GetStatusCodeName(byte code	///< One of the StatusCode enums.
						      ) {
  switch (code) {
 ...
 * @return PICC_Type
 */
byte CMFRC522::PICC_GetType(byte sak		///< The SAK byte returned from PICC_Select().
			   ) {
 ...
 * 
 */
const string CMFRC522::PICC_GetTypeName(byte piccType	///< One of the PICC_Type enums.
						     ) {
  switch (piccType) {
 ...
 */
void CMFRC522::MIFARE_SetAccessBits(	byte *accessBitBuffer,	///< Pointer to byte 6, 7 and 8 in the sector trailer. Bytes [0..2] will be set.
 ...
 */
bool CMFRC522::MIFARE_SetUid(byte *newUid, byte uidSize, bool logErrors) {
  ...
  byte status = PCD_Authenticate(CMFRC522::PICC_CMD_MF_AUTH_KEY_A, (byte)1, &key, &uid);
  if (status != STATUS_OK) {
	...
	return false;
      ...
      status = PCD_Authenticate(CMFRC522::PICC_CMD_MF_AUTH_KEY_A, (byte)1, &key, &uid);
      if (status != STATUS_OK) {
